...
module Requests.Details {
    ...
    function bind(
        ...
        $(() => {
            ...
            var currentTask: Dns.Interfaces.ITaskDTO = ko.utils.arrayFirst(tasks, (item) => { return item.WorkflowActivityID == request.CurrentWorkFlowActivityID() && item.EndOn == null; });
            ...
            var taskID: any = Global.GetQueryParam("TaskID");
            ...
            var viewOverview = ko.utils.arrayFirst(screenPermissions, p => p.toLowerCase() == Permissions.ProjectRequestTypeWorkflowActivities.ViewRequestOverview.toLowerCase()) != null;
            var viewComments = ko.utils.arrayFirst(screenPermissions, p => p.toLowerCase() == Permissions.ProjectRequestTypeWorkflowActivities.ViewComments.toLowerCase()) != null;
            var viewTask = ko.utils.arrayFirst(screenPermissions, p => p.toLowerCase() == Permissions.ProjectRequestTypeWorkflowActivities.ViewTask.toLowerCase()) != null;
            var viewDocuments = ko.utils.arrayFirst(screenPermissions, p => p.toLowerCase() == Permissions.ProjectRequestTypeWorkflowActivities.ViewDocuments.toLowerCase()) != null;
            var assignRequestNotifications = ko.utils.arrayFirst(screenPermissions, p => p.toLowerCase() == Permissions.Request.AssignRequestLevelNotifications.toLowerCase()) != null;
            ...
            var activityCommentsVM = viewComments ? Controls.WFComments.List.init($('#Comments'), screenPermissions, rovm.Request.ID(), request.CurrentWorkFlowActivity(), request.CurrentWorkFlowActivityID()) : null;
            //init all comments for request; user needs to have permission to view the overview
            var overallCommentsVM = (viewOverview) ? Controls.WFComments.List.init($('#OverallComments'), screenPermissions, rovm.Request.ID(), null, null) : null;
            ...
            if (viewDocuments) {
                ...
                var activityDocumentsVM: Controls.WFDocuments.List.ViewModel = null;
                ...
                var overallDocumentsVM: Controls.WFDocuments.List.ViewModel = null;
