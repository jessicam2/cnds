        ...
        function init() {
            ...
            $.when(id == null ? null : Dns.WebApi.Projects.GetPermissions([id], [
                ...
                if (project != null && project.GroupID) {
                    ...
                    //this is completely separate because we don't want to have to wait for document complete to be able to execute so we have to be repetative.
                    $(function () {
                        var bindingControl = $("#Content");
                        vm = new ViewModel(screenPermissions || [Permissions.Project.Edit,
                            Permissions.Project.ManageSecurity,
                            Permissions.Project.ManageRequestTypes], project, [], projectRequestTypeWorkflowActivityAcls || [], groups, dataMartList || [], [], permissionList, [], projectDataMarts || [], projectDataMartPermissions, projectPermissions, projectRequestTypePermissions, projectDataMartRequestTypePermissions, projectEventPermissions, projectDataMartEventPermissions, projectOrganizationPermissions, projectOrganizationEvents, securityGroupTree, projectSecurityGroups || [], projectOrganizations || [], eventList || [], projectDataMartsEventList || [], groupid, requestTypeList, fieldOptions, bindingControl);
                        ko.applyBindings(vm, bindingControl[0]);
                    });
                }
            });
