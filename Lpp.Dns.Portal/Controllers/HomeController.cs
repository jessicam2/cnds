    ...
    public class HomeController : BaseController
    {
        ...
        public async Task<ActionResult> Login(string username, string password, string returnUrl)
        {
            ...
            using (var client = new Lpp.Dns.ApiClient.DnsClient(WebConfigurationManager.AppSettings["ServiceUrl"]))
            {
                ...
                //put the CNDS ID for the user in the Session if it exists
                var cndsUserID = DataContext.UserSettings.Where(s => s.UserID == contact.ID && s.Key == "CNDS.ID").Select(s => s.Setting).FirstOrDefault();
                if (!string.IsNullOrEmpty(cndsUserID))
                {
                    Guid cndsID;
                    if(Guid.TryParse(cndsUserID, out cndsID))
                    {
                        Session["CNDS.UserID"] = cndsID;
                    }
                }
