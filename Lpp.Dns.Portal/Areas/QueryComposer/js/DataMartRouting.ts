module Plugins.Requests.QueryBuilder.DataMartRouting {
    ...
    export class ViewModel extends Global.PageViewModel {
        ...
        ExternalRoutes: KnockoutObservableArray<Plugins.Requests.QueryBuilder.DataMartRouting.Routings>;
        LoadDataMarts: (ProjectID: any, strQuery: string) => void;
        public DataMartsBulkEdit: () => void;
        public SelectedRoutings: (forSubmit: boolean) => Dns.Interfaces.IRequestDataMartDTO[];
        ...
        constructor(
            ...
            this.ExternalRoutes = ko.observableArray<Plugins.Requests.QueryBuilder.DataMartRouting.Routings>();

            //only get the external routes if in request workflow
            if (window.location.pathname.toLowerCase().indexOf('/requests/') === 0) {
                let requestID = Global.GetQueryParam("ID");
                if (requestID) {
                    Dns.WebApi.Requests.GetExternalRoutesForRequest(requestID).done((result) => {
                        var externalRoutes = [];
                        ...
                        for (let i = 0; i < result.length; i++) {
                            ...
                            let existingRT = ko.utils.arrayFirst(self.ExistingRequestDataMarts, (r) => r.DataMartID == rt.ID);
            ...
            this.DataMartsSelectAll = () => {
                ...
                ko.utils.arrayForEach(self.ExternalRoutes(), (rt) => {
            ...
            this.SelectedRoutings = (forSubmit: boolean) => {
                ...
                if (forSubmit) {
                    ...
                    ko.utils.arrayForEach(self.ExternalRoutes(), (route) => {
            ...
            this.DataMartsBulkEdit = () => {
                let dueDate = self.DefaultDueDate() != null ? new Date(self.DefaultDueDate().getTime()) : null;
                    ...
                    .done((result: any) => {
                        if (result != null) {
                            ...
                            if (result.UpdatePriority || result.UpdateDueDate) {
                                ...
                                ko.utils.arrayFilter(self.ExternalRoutes(), (route) => {
                                    return self.SelectedDataMartIDs.indexOf(route.DataMartID) > -1;
                                }).forEach((route) => {
                                    if (priority != null)
                                        route.Priority(priority);
        ...
        public UpdateRoutings(updates) {
            ...
            this.ExternalRoutes().forEach((dm) => {
                if (newPriority != null) {
                    dm.Priority(newPriority);
                }
                if (newDueDate != null) {
