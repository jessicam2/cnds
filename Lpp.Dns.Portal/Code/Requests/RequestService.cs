        ...
        public DnsResult AddDataMarts(IRequestContext ctx, IEnumerable<IDnsDataMart> dataMarts)
        {
            ...
            try
            {
                using (var datacontext = new DataContext())
                {
                    ...
                    //Determine is initial status is submitted based on permission setting
                    var targetStatus = RoutingStatus.Draft;
                    if (AsyncHelpers.RunSync<bool>(() => datacontext.HasPermissions<Request>(Auth.ApiIdentity, request, PermissionIdentifiers.Request.SkipSubmissionApproval)))
