...
namespace Lpp.Dns.DataMart.Model
{
    ...
    public class FileDistributionModelProcessor : IModelProcessor
    {
        ...
        {
            responseDocuments.Add( new InternalDocument(new Document( Guid.NewGuid().ToString(), GetMimeType( filePath ), Path.GetFileName(filePath)), filePath, new FileInfo(filePath).Length));
