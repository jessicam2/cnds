    ...
    public class RequestReview : ActivityBase<Request>
    {
        ...
        public async override Task<ValidationResult> Validate(Guid? activityResultID)
        {
            ...
            if (activityResultID != SaveResultID)
            {
                if (_entity.SubmittedOn == null)
                    errors.AppendHtmlLine("Cannot approve or reject a request that does not have a submitted on date.");
        ...
        {   
            ...
            if (activityResultID.Value == ApproveResultID)
            {
                ...
                {
                    document = await db.Documents.Where(x => x.ItemID == previousTask.ID).FirstOrDefaultAsync();
                }
                ...
                foreach (var dm in _entity.DataMarts.Where(dm => dm.Status == DTO.Enums.RoutingStatus.AwaitingRequestApproval))
                {
                    ...
                    if (document != null && !(await db.RequestDocuments.AnyAsync(d => d.RevisionSetID == document.RevisionSetID && d.ResponseID == currentResponse.ID)))
                    {
                    ...
                    if (await db.Requests.Where(r => r.ID == _entity.ID && r.CreatedBy.UserType == DTO.Enums.UserTypes.CNDSNetworkProxy).AnyAsync())
                    {
                        await UpdateCrossNetworkRoutingStatusForSourceRequest(await db.Responses.Where(rsp => rsp.RequestDataMart.RequestID == _entity.ID && rsp.Count == rsp.RequestDataMart.Responses.Max(rrsp => rrsp.Count) && (rsp.RequestDataMart.RoutingType == DTO.Enums.RoutingType.SourceCNDS || rsp.RequestDataMart.RoutingType == DTO.Enums.RoutingType.ExternalCNDS)).Select(rsp => new DTO.CNDSUpdateRoutingStatusDTO { ResponseID = rsp.ID, Message = rsp.ResponseMessage, RoutingStatus = DTO.Enums.RoutingStatus.Submitted }).ToArrayAsync());
                    }
                }
                ...
                //if the request is cross-network (created by a network proxy user), notify source request of routing status changes
                if (_entity.DataMarts.Any(dm => dm.RoutingType == DTO.Enums.RoutingType.SourceCNDS) && _entity.Status == DTO.Enums.RequestStatuses.Submitted)
                {
            ...
            else if (activityResultID.Value == RejectResultID)
            {
                ...
                //if the request is cross-network, notify source request of routing/request cancelation
                if (await db.Requests.Where(r => r.ID == _entity.ID && r.CreatedBy.UserType == DTO.Enums.UserTypes.CNDSNetworkProxy).AnyAsync())
                {
                    await UpdateCrossNetworkRoutingStatusForSourceRequest(await db.Responses.Where(rsp => rsp.RequestDataMart.RequestID == _entity.ID && rsp.Count == rsp.RequestDataMart.Responses.Max(rrsp => rrsp.Count) && (rsp.RequestDataMart.RoutingType == DTO.Enums.RoutingType.SourceCNDS || rsp.RequestDataMart.RoutingType == DTO.Enums.RoutingType.ExternalCNDS)).Select(rsp => new DTO.CNDSUpdateRoutingStatusDTO { ResponseID = rsp.ID, Message = rsp.ResponseMessage, RoutingStatus = rsp.RequestDataMart.Status }).ToArrayAsync());
                }
