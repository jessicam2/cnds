...
namespace Lpp.Dns.Workflow.Default.Activities
{
    public class ApproveResponse : ActivityBase<Request>
    {
        ...
        {
            get { return DefaultWorkflowConfiguration.ApproveResponseID; }
        }
        ...
        public override async Task<CompletionResult> Complete(string data, Guid? activityResultID)
        {
            ...
            if (activityResultID.Value == ApproveResultID) //Approve
            {
                ...
                if(Response.RequestDataMart.RoutingType == DTO.Enums.RoutingType.ExternalCNDS)
                {
                    var returnResponse = new List<CNDSUpdateRoutingStatusDTO>();
                    returnResponse.Add(new CNDSUpdateRoutingStatusDTO { ResponseID = Response.ID, RoutingStatus = DTO.Enums.RoutingStatus.Completed });
                    await PostResponseDocuments(Response.ID);
                    await UpdateCrossNetworkRoutingStatusForSourceRequest(returnResponse);
                }
            }
