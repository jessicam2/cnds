...
namespace Lpp.Dns.Workflow.Default.Activities
{
    ...
    public class ViewRequest : ActivityBase<Request>
    {
        ...
        public override async Task<Lpp.Workflow.Engine.ValidationResult> Validate(Guid? activityResultID)
        {
            ...
            var createdBy = await db.Users.Where(x => x.ID == _entity.CreatedByID).FirstOrDefaultAsync();
            if (createdBy.UserType == DTO.Enums.UserTypes.CNDSNetworkProxy && (activityResultID.Value == AddDataMartsResultID || activityResultID.Value == RemoveDataMartsResultID))
            {
                return new ValidationResult
                {
                    Success = false,
                    Errors = CommonMessages.CannnotAddRoutesToCNDSRequest
                };
            }
        ...
        public override async Task<Lpp.Workflow.Engine.CompletionResult> Complete(string data, Guid? activityResultID)
        {
            ...
            else if (activityResultID.Value == EditRoutingStatusResultID)
            {
                ...
                var updateParticipants = new List<DTO.CNDSUpdateRoutingStatusDTO>();
                var updateSources = new List<DTO.CNDSUpdateRoutingStatusDTO>();
                foreach (var requestDataMart in requestDataMarts)
                {
                    ...
                    foreach (var datamart in dataMarts)
                    {
                        if (datamart.RequestDataMartID == requestDataMart.ID)
                        {
                            ...
                            if (requestDataMart.RoutingType == DTO.Enums.RoutingType.SourceCNDS)
                            {
                                updateParticipants.Add(new DTO.CNDSUpdateRoutingStatusDTO
                                {
                                    ResponseID = currentResponse.ID,
                                    Message = datamart.Message,
                                    RoutingStatus = datamart.NewStatus,
                                    NetworkID = networkID
                                });
                            }
                            if (requestDataMart.RoutingType == DTO.Enums.RoutingType.ExternalCNDS)
                            {
                                updateSources.Add(new DTO.CNDSUpdateRoutingStatusDTO
                                {
                                    ResponseID = currentResponse.ID,
                                    Message = datamart.Message,
                                    RoutingStatus = datamart.NewStatus,
                ...
                if(updateParticipants.Count() > 0)
                {
                    var resp = await UpdateParticipantRoutingStatusForSourceRequest(updateParticipants);
                ...
                {
                    var resp = await UpdateCrossNetworkRoutingStatusForSourceRequest(updateSources);
                    if (!resp.IsSuccessStatusCode)
                    {
                        throw new Exception(resp.Content.ToString());
                    }
            ...
            else if (activityResultID.Value == ResubmitResultID)
            {
                ...
                var updateParticipants = new List<DTO.CNDSResubmitRouteDTO>();
                var updateSources = new List<DTO.CNDSResubmitRouteDTO>();
                ...
                foreach (var dm in datamarts)
                {
                    ...
                    if (dm.RoutingType == DTO.Enums.RoutingType.SourceCNDS)
                        updateParticipants.Add(new DTO.CNDSResubmitRouteDTO { ResponseID = response.ID, RequestDatamartID = dm.ID, Message = resubmitModel.ResubmissionMessage});
                    if (dm.RoutingType == DTO.Enums.RoutingType.ExternalCNDS)
                        updateSources.Add(new DTO.CNDSResubmitRouteDTO { ResponseID = response.ID, RequestDatamartID = dm.ID, Message = resubmitModel.ResubmissionMessage });
                    foreach (var revset in revisionSetIDs)
                ...
                if(updateParticipants.Count() > 0)
                {
                    var resp = await ResubmitRouteToParticipant(updateParticipants);
                    if (!resp.IsSuccessStatusCode)
                    {
                        throw new Exception(resp.Content.ToString());
                    }
                ...
                if (updateSources.Count() > 0)
                {
                    var resp = await ResubmitRouteToSource(updateSources);
                    if (!resp.IsSuccessStatusCode)
                    {
                        throw new Exception(resp.Content.ToString());
                    }
        ...
        public async Task LogTaskModified(string optionalMessage = null)
        {
            var task = await PmnTask.GetActiveTaskForRequestActivityAsync(_entity.ID, ID, db);
            if (task == null)
            {
