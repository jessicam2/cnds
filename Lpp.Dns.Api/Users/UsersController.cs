...
namespace Lpp.Dns.Api.Users
{
    ...
    public partial class UsersController : LppApiDataController<User, UserDTO, DataContext, PermissionDefinition>
    {
        ...
        public override async Task<UserDTO> Get(Guid id)
        {
            ...
            var user = await DataContext.Users.Where(o => o.ID == id).AsNoTracking().Map<User, UserDTO>().FirstOrDefaultAsync();
            //TODO: create helper to cache
            if (user.Active && user.UserType == UserTypes.User)
            {
                ...
                try
                {
                    using (var cnds = new CNDSEntityUpdater(networkID))
                    {
                        ...
                        cndsUserID = response.Select(org => org.EntityID).FirstOrDefault();
                        ...
                        var availUserMetadata = await CNDSEntityUpdater.CNDS.Domain.List("$filter=EntityType eq Lpp.CNDS.DTO.Enums.EntityType'1'");
                        var currentUserMetadata = await CNDSEntityUpdater.CNDS.Users.ListUserDomains(cndsUserID);
                        var visibility = await CNDSEntityUpdater.CNDS.Users.GetDomainVisibility(cndsUserID);

                        List<MetadataDTO> meta = new List<MetadataDTO>();
                        foreach (var metadata in availUserMetadata.Where(x => x.ParentDomainID == null))
                        {
                            meta.Add(cnds.GetMetadataChildren(metadata.ID, availUserMetadata, currentUserMetadata, visibility, cndsUserID));
                        }
                        user.Metadata = meta;
                    }
                }
        ...
        /// <summary>
		/// Returns All Available User Metadata
		/// </summary>
		/// <param></param>
		/// <returns></returns>
		[HttpGet]
        public async System.Threading.Tasks.Task<IEnumerable<MetadataDTO>> GetAvailableUserMetadata()
        {
            IList<MetadataDTO> userMetadata = new List<MetadataDTO>();
            Guid networkID = await CNDSEntityUpdater.GetNetworkID(DataContext);
            try
            {
                using (var cnds = new CNDSEntityUpdater(networkID))
                {
                    ...
                    foreach (var metadata in availUserMetadata.Where(x => x.ParentDomainID == null))
                    {
                        userMetadata.Add(cnds.GetMetadataChildren(metadata.ID, availUserMetadata, new List<cndsDTO.DomainDataDTO>(), null, null));
                    }
                }
            }
            catch (Exception ex)
        ...
        public async Task<UserDTO> ValidateLogin(LoginDTO login)
        {
            ...
            if(contact.Active)
            {
                var cndsUserSettings = await DataContext.UserSettings.Where(s => s.UserID == user.ID && (s.Key == "CNDS.ID" || s.Key == "CNDS.OrganizationID")).ToArrayAsync();

                Dictionary<string,Guid> pmnIDs = new Dictionary<string,Guid>();
                if(!cndsUserSettings.Any(s => s.Key == "CNDS.ID"))
                {
                    pmnIDs.Add("CNDS.ID", contact.ID);
                }
                if(!cndsUserSettings.Any(s => s.Key == "CNDS.OrganizationID"))
                {
                    pmnIDs.Add("CNDS.OrganizationID", contact.OrganizationID.Value);
                }

                if(pmnIDs.Keys.Count > 0)
                {
                    //get the CNDS ID and save as setting
                    using (var cndsUpdater = new CNDSEntityUpdater(await CNDSEntityUpdater.GetNetworkID(DataContext)))
                    {
                        var cndsIdentifiers = (await Lpp.CNDS.ApiClient.CNDSEntityUpdater.GetCNDSEntityIdentifiers(pmnIDs.Values.ToArray())).ToArray();
                        if (cndsIdentifiers.Any())
                        {
                            foreach (var pmn in pmnIDs)
                            {
                                var cID = cndsIdentifiers.FirstOrDefault(s => s.NetworkEntityID == pmn.Value);
                                if (cID != null)
                                {
                                    DataContext.UserSettings.Add(new UserSetting { UserID = contact.ID, Key = pmn.Key, Setting = cID.EntityID.ToString("D") });
                                }
                            }
        ...
        public async Task<HttpResponseMessage> ChangePassword(UpdateUserPasswordDTO updateInfo)
        {
            ...
            if(user.UserType == UserTypes.CNDSNetworkProxy)
            {
                throw new NotSupportedException("Changing or adding a password for a CNDS Network Proxy user is not supported.");
            }
        ...
        public async Task<IEnumerable<MenuItemDTO>> ReturnMainMenu()
        {
            ...
            var permissions = await DataContext.HasGrantedPermissions(Identity, PermissionIdentifiers.DataMart.RunAuditReport,
                ...
                PermissionIdentifiers.Portal.SearchCNDS);
            ...
            var cnds = new List<MenuItemDTO>();

            bool checkManagePermissionsPermission = false;
            bool checkManageMetadataPermission = false;
            bool checkManageCrossNetworkRequestTypeMappings = false;            

            var cndsUserID = Guid.Empty;
            Guid networkID = await CNDSEntityUpdater.GetNetworkID(DataContext);
            try
            {
                using (var cndsUpdater = new CNDSEntityUpdater(networkID))
                {
                    ...
                    using(var permissionsHelper = new CNDSPermissions())
                    {
                        var grantedPermissions = await permissionsHelper.GetAllowedPermissionsForUser(cndsUserID);

                        checkManagePermissionsPermission = grantedPermissions.Contains(Lpp.CNDS.DTO.Security.PermissionIdentifiers.Global.ManageSecurity.ID);
                        checkManageMetadataPermission = grantedPermissions.Contains(Lpp.CNDS.DTO.Security.PermissionIdentifiers.Global.ManageMetadata.ID);
                        checkManageCrossNetworkRequestTypeMappings = grantedPermissions.Contains(Lpp.CNDS.DTO.Security.PermissionIdentifiers.Global.ManageRequestTypeMappings.ID);
                    }
                }
            }
            catch (Exception ex)
            ...
            if (checkManageMetadataPermission)
            {
                cnds.Add(new MenuItemDTO
                {
                    text = "Manage Metadata",
                    url = "/cnds/managemetadata/"
                });
            }

            if (checkManageCrossNetworkRequestTypeMappings)
            {
                cnds.Add(new MenuItemDTO { text = "Manage Request Type Mappings", url = "/cnds/managerequesttypes/" });
            }

            if (permissions.Contains(PermissionIdentifiers.Portal.SearchCNDS))
            {
                cnds.Add(new MenuItemDTO
                {
                    text = "New Request",
            ...
            if (checkManagePermissionsPermission)
            {
                cnds.Add(new MenuItemDTO
                {
                    text = "Permissions",
                    url = "/cnds/managepermissions/"
                });
            }
            ...
            if (permissions.Contains(PermissionIdentifiers.Portal.SearchCNDS))
            {
                cnds.Add(new MenuItemDTO
                {
                    ...
                    url = "#",
                    items = new[] {
                        new MenuItemDTO { text = "Data Sources", url = "/cnds/search/datasources/" },
                        new MenuItemDTO { text = "Organizations", url = "/cnds/search/organizations/" }
                    }
                });
            }
        ...
        public override async Task Delete([FromUri]IEnumerable<Guid> ID)
        {
            ...
            if(users.Any(u => u.UserType == UserTypes.CNDSNetworkProxy))
            {
                throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Deleting of CNDS Network Proxy Users is not supported."));
            }
            ...
            if (CNDSEntityUpdater.CanUpdateCNDS)
            {
                try
                {
                    using (var cnds = new CNDSEntityUpdater(networkID))
                    {
                        var networkEntities = await CNDSEntityUpdater.GetCNDSEntityIdentifiers(users.Select(x => x.ID).ToArray());
                        foreach (var user in users)
                        {
                            await CNDSEntityUpdater.CNDS.Users.Delete(networkEntities.Where(x => x.NetworkEntityID == user.ID).Select(x => x.EntityID).FirstOrDefault());
                            user.Deleted = true;
        ...
        public override async Task<IEnumerable<UserDTO>> InsertOrUpdate(IEnumerable<UserDTO> values)
        {
            ...
            var userDTO = values.FirstOrDefault();
            if (userDTO.ID == null || userDTO.ID == Guid.Empty)
                userDTO.ID = Lpp.Utilities.DatabaseEx.NewGuid();
            var user = DataContext.Users.Find(userDTO.ID);

            #region CNDS
            if ((user != null && user.UserType != UserTypes.CNDSNetworkProxy) && ((user != null && userDTO.Active) || (user != null && !userDTO.Active && user.Active)))
            {
                Guid networkID = await CNDSEntityUpdater.GetNetworkID(DataContext);
                if (CNDSEntityUpdater.CanUpdateCNDS)
                {
                    try
                    {
                        using (var cnds = new CNDSEntityUpdater(networkID))
                        {
                            await CNDSEntityUpdater.RegisterOrUpdateUsers(userDTO);
                        }
                    }
                    catch (Exception ex)
                    {
            ...
            if (user == null)
            {
                user = new User
                {
                    ...
                    RejectReason = userDTO.RejectReason,
                    DeactivationReason = userDTO.DeactivationReason,
                    OrganizationRequested = userDTO.OrganizationRequested,
                    RoleRequested = userDTO.RoleRequested
                };
                if (userDTO.ActivatedOn.HasValue)
                    user.ActivatedOn = userDTO.ActivatedOn.Value.LocalDateTime;
                else
                    user.ActivatedOn = null;

                if (userDTO.SignedUpOn.HasValue)
                    user.SignedUpOn = userDTO.SignedUpOn.Value.LocalDateTime;
                else
                    user.SignedUpOn = null;

                if (userDTO.RejectedOn.HasValue)
                    user.RejectedOn = userDTO.RejectedOn.Value.LocalDateTime;
                else
                    user.RejectedOn = null;

                if (userDTO.DeactivatedOn.HasValue)
                    user.DeactivatedOn = userDTO.DeactivatedOn.Value.LocalDateTime;
                else
                    user.DeactivatedOn = null;

                if (userDTO.DeactivatedByID.HasValue)
                    user.DeactivatedByID = userDTO.DeactivatedByID.Value;
                else
            ...
            else
            {
                ...
                user.RejectReason = userDTO.RejectReason;
                user.DeactivationReason = userDTO.DeactivationReason;
                user.OrganizationRequested = userDTO.OrganizationRequested;
                user.RoleRequested = userDTO.RoleRequested;
            ...
            #endregion CNDS
            IList<UserDTO> returnUser = new List<UserDTO>();
            returnUser.Add(userDTO);

            await CompleteRegistrationTask(returnUser);
        ...
        public override async Task<IEnumerable<UserDTO>> Update(IEnumerable<UserDTO> values)
        {
            ...
            #region CNDS
            if (userDTO.Active && userDTO.UserType != UserTypes.CNDSNetworkProxy)
            {
                Guid networkID = await CNDSEntityUpdater.GetNetworkID(DataContext);
                if (CNDSEntityUpdater.CanUpdateCNDS)
                {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task UpdateUserVisibility(IEnumerable<MetadataDTO> userMetadata)
        {
            Guid networkID = await CNDSEntityUpdater.GetNetworkID(DataContext);
            using (var cnds = new CNDSEntityUpdater(networkID))
            {
                await CNDSEntityUpdater.RegisterOrUpdateUserDomainVisibility(userMetadata);
            }
        }
