...
namespace Lpp.Dns.Api.Requests
{
    ...
    public class RequestObserversController : LppApiDataController<RequestObserver, RequestObserverDTO, DataContext, PermissionDefinition>
    {
        ...
        {
            List<ObserverEventDTO> lstEvents = new List<ObserverEventDTO>();
            lstEvents.Add(new ObserverEventDTO() { ID = Lpp.Dns.DTO.Events.EventIdentifiers.Request.NewRequestSubmitted.ID, Name = "New Request Submitted"});
            lstEvents.Add(new ObserverEventDTO() { ID = Lpp.Dns.DTO.Events.EventIdentifiers.Request.RequestAssignmentChange.ID, Name = "Request Assigned"});
            lstEvents.Add(new ObserverEventDTO() { ID = Lpp.Dns.DTO.Events.EventIdentifiers.Request.RequestCommentChange.ID, Name = "New Comment on Request"});
            lstEvents.Add(new ObserverEventDTO() { ID = Lpp.Dns.DTO.Events.EventIdentifiers.Document.Change.ID, Name = "New Document Attached to Request"});
            lstEvents.Add(new ObserverEventDTO() { ID = Lpp.Dns.DTO.Events.EventIdentifiers.Request.RequestStatusChanged.ID, Name = "Request Status Changed"});
            lstEvents.Add(new ObserverEventDTO() { ID = Lpp.Dns.DTO.Events.EventIdentifiers.Request.RoutingStatusChanged.ID, Name = "Request DataMart Routing Status Changed"});

            return lstEvents.OrderBy(p => p.Name);
        }
        ...
        public HttpResponseMessage ValidateInsertOrUpdate(System.Collections.Generic.IEnumerable<Lpp.Dns.DTO.RequestObserverDTO> values)
        {
            ...
            if (values.Where(x => x.SecurityGroupID.HasValue).Any())
            {
                ...
                {
                    errors.Add(string.Format("\"{0}\" does not have permission to subscribe to \"{1}\".", invalid.SecurityGroupPath, invalid.EventName));
                }
            }
            ...
            if (values.Where(x => x.UserID.HasValue).Any())
            {
                ...
                foreach (var u in invalidUsers.Distinct().GroupBy(k => new { k.EventID, k.EventName, k.UserID, k.FirstName, k.LastName, k.Username }))
                {
                    ...
                    errors.Add(string.Format("\"{0}\" does not have permission to subscribe to \"{1}\".", name, u.Key.EventName));
                }
            }
